...
def main():
    ...
    profiler = PyTorchProfiler(                        # ←② NEW
    dirpath="./profiler_logs",                      # trace 保存目录
    filename="trace",                               # 输出文件名 trace.json
    schedule=torch.profiler.schedule(
        wait=1, warmup=1, active=3, repeat=2        # 周期：跳1、热身1、记录3、重复2次
    ),
    on_trace_ready=torch.profiler.tensorboard_trace_handler(
        "./profiler_logs/tb"                        # 也可直接让 TensorBoard 读取
    ),
