...
for cls in (bnb.nn.Linear4bit, bnb.nn.Linear8bitLt):
    cls.to = _noop_to
...
def main():
    ...
    dirpath="./profiler_logs",                      # trace Save directory
    filename="trace",                               # Output filename trace.json
    schedule=torch.profiler.schedule(
        wait=1, warmup=1, active=3, repeat=2        # Cycle: skip 1, warm up 1, record 3, repeat 2 times
    ),
    on_trace_ready=torch.profiler.tensorboard_trace_handler(
        "./profiler_logs/tb"                        # Or you can just let the TensorBoard read it.
    ),
    ...
    checkpoint_callback = ModelCheckpointEx(
        ...
        every_n_epochs=1,
        lora_args=lora_args,
        # monitor="loss",mode = "min", save_top_k = 10 Store 10 models by loss
    ...
    trainer = Trainer(
        ...
        # can try "32": "32-true", "16": "16-mixed", "bf16": "bf16-mixed"
        precision=precision,
        profiler=profiler,
